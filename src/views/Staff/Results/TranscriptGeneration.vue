<template>
    <div class="min-h-screen bg-gray-50 p-4">
        <div class="max-w-7xl mx-auto space-y-6">
                ...
                <template #content>
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <Button @click="$router.push({ name: 'staff-results' })" icon="pi pi-arrow-left"
                                label="Back to Results" class="p-button-secondary" size="small" />
                            <Button @click="showBulkGenerationModal = true" icon="pi pi-file-export"
                                label="Bulk Generate" class="p-button-success" size="small" />
                        </div>
                    </div>
                </template>
            </Card>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="bg-blue-50 p-4 rounded-lg text-center">
                                <div class="text-2xl font-bold text-blue-600">{{ (selectedStudent.cgpa || 0).toFixed(2)
                                    }}</div>
                                <div class="text-sm text-blue-700">Current CGPA</div>
                            </div>
                            <div class="bg-green-50 p-4 rounded-lg text-center">
                                <div class="text-2xl font-bold text-green-600">{{ selectedStudent.total_credit_units ||
                                    0 }}</div>
                                <div class="text-sm text-green-700">Total Units</div>
                            </div>
                            <div class="bg-purple-50 p-4 rounded-lg text-center">
                                <div class="text-2xl font-bold text-purple-600">{{ selectedStudent.completed_semesters
                                    || 0 }}</div>
                                <div class="text-sm text-purple-700">Semesters</div>
                            </div>
                            <div class="bg-yellow-50 p-4 rounded-lg text-center">
                                <div class="text-sm font-bold text-yellow-600">{{ getDegreeClass(selectedStudent.cgpa)
                                    }}</div>
            ...
            <div v-if="selectedStudent && academicRecords.length > 0"
                class="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 border border-gray-100">
            ...
            <!-- Search Results -->
            <div v-if="searchResults.length > 0 && !selectedStudent"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="student in searchResults" :key="student.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button @click="selectStudent(student)" class="text-blue-600 hover:text-blue-900">
